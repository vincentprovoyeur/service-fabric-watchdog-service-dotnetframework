...
namespace Microsoft.ServiceFabric.WatchdogService
{
    ...
    internal sealed class HealthCheckOperations : IDisposable
    {
        ...
        internal async Task<HealthCheck> ExecuteHealthCheckAsync(HealthCheck hc, Partition partition)
        {
            ...
            try
            {
                ...
                if ((null != hc.WarningStatusCodes) && (hc.WarningStatusCodes.Contains((int) response.StatusCode)))
                {
                    ...
                    success = false;
                    ServiceEventSource.Current.Trace("ExecuteHealthCheckAsync: INFO: request returned WARNING for service" + uri.AbsoluteUri);
                }
                else if ((null != hc.ErrorStatusCodes) && (hc.ErrorStatusCodes.Contains((int) response.StatusCode)))
                {
                    hs = HealthState.Error;
                    success = false;
                    ServiceEventSource.Current.Trace("ExecuteHealthCheckAsync: INFO: request returned ERROR for service" + uri.AbsoluteUri);
                }
                ...
                else if (true == response.StatusCode.IsSuccessCode())
                {
                    ServiceEventSource.Current.Trace("ExecuteHealthCheckAsync: INFO: request returned SUCCESS for service" + uri.AbsoluteUri);
                }

                // Report health result to Service Fabric.
                ServiceEventSource.Current.Trace("ExecuteHealthCheckAsync: INFO: Report health result to Service Fabric by calling ServiceFabricClient.HealthManager.ReportHealth");
                this.Client.HealthManager.ReportHealth(new PartitionHealthReport(hc.Partition, new HealthInformation("Watchdog Health Check" + hc.Name, hc.Name, hs)));
                ...
                ServiceEventSource.Current.Trace("ExecuteHealthCheckAsync: INFO: Report the availability of the tested service to the telemetry provider.");

                // Return a new HealthCheck instance containing the results of the request.
                ServiceEventSource.Current.Trace("ExecuteHealthCheckAsync: INFO: Return a new HealthCheck instance containing the results of the request. Status Code: [" + response.StatusCode + "]");
        ...
        public async Task<bool> AddHealthCheckAsync(HealthCheck hcm)
        {
            ...
            {
                ServiceEventSource.Current.Trace($"PostHealthCheck:AddHealthCheckAsync ERROR: SERVICE NAME [" + hcm.ServiceName.AbsoluteUri + "] DOES NOT EXIST WITHIN THE CLUSTER");
            ...
            try
            {
                ...
                using (ITransaction tx = this._service.StateManager.CreateTransaction())
                {
                    ...
                    ServiceEventSource.Current.Trace($"PostHealthCheck:AddHealthCheckAsync INFO: adding healthcheck item in the dictionary with key =" + hcm.Key);
                    ...
                    ServiceEventSource.Current.Trace($"PostHealthCheck:AddHealthCheckAsync INFO: Create the HealthCheckScheduleItem instance and save it.");
